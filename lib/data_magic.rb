...
module DataMagic
  ...
  def self.nested_object_type(hash)
    hash.each do |key, value|
      next unless value.is_a?(Hash) && value[:type].nil?
      hash[key] = {
        ...
        settings: {
          analysis: {
            ...
            analyzer: {
              autocomplete_index: {
                tokenizer: 'standard',
                filter: %w(lowercase stop autocomplete_filter),
                type: 'custom'
              },
              autocomplete_search: {
                tokenizer: 'standard',
  ...
  def self.index_name_from_options(options)
    ...
    logger.info "WARNING: DataMagic.search options api will override index, only one expected" if api && options[:index]
    if api
      index_name = config.find_index_for(api)
      if index_name.nil?
        fail ArgumentError, "no configuration found for '#{api}', available endpoints: #{self.config.api_endpoint_names.inspect}"
      end
    else
  ...
  def self.index_data_if_needed
    ...
    if @index_thread && @index_thread.alive?
  ...
  def self.init(options = {})
    ...
    if self.config.nil? # only init once
      ...
      @index_thread.join if options[:load_now] && @index_thread
    end
  end # init

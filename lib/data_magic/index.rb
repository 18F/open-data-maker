...
module DataMagic
  ...
  def self.get_id(row, options = {})
    if config.data['unique'].length > 0
      ...
      config.data['unique'].each { |key| row.delete key } if options[:remove]
    else
      result = nil
    end
    result
  ...
  def self.import_csv(data, options = {})
    ...
    logger.info "  options: #{options.reject { |k, _v| k == :mapping }.to_yaml}"
    ...
    begin
      CSV.parse(
        ...
        if options[:nest].nil? # first time or normal case
          client.index(index: es_index_name,
                       id: get_id(doc),
                       type: 'document',
                       body: doc)
        else
          begin
            ...
            client.update(index: es_index_name,
                          id: id,
                          type: 'document',
                          body: { doc: doc })
        ...
        num_rows += 1
        if options[:limit_rows] && num_rows == options[:limit_rows]
    ...
    fail InvalidData, "zero rows" if num_rows == 0
  ...
  def self.import_with_dictionary(options = {})
    ...
    config.files.each_with_index do |filepath, index|
      ...
      begin
        ...
        rows, = DataMagic.import_csv(data, options)
